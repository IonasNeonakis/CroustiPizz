...
namespace CroustiPizz.Mobile.ViewModels
{
    public class ShopListViewModel : ViewModelBase
    {
        ...
        public override async Task OnResume()
        {
            ...
            if (response.IsSuccess)
            {
                ...
                foreach (ShopItem shopItem in Shops)
                {
                    ...
                    {
                        shopItem.Distance = GetDistance(shopItem.Longitude, shopItem.Latitude, _userLocation.Longitude,
        ...
        private int DistancePoint(ShopItem item1, ShopItem item2)
        {
            ...
            ) // si la position de l'utilisateur n'est pas donnée alors on laisse la liste tel quelle
            {
                return String.Compare(item1.Name, item2.Name, StringComparison.Ordinal);
            }

            double distanceX = GetDistance(item1.Longitude, item1.Latitude, _userLocation.Longitude,
                _userLocation.Latitude);
            double distanceY = GetDistance(item2.Longitude, item2.Latitude, _userLocation.Longitude,
                _userLocation.Latitude);
